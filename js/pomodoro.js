 ...
 * Complete Pomodoro technique implementation
 */
...
class PomodoroTimer {
    ...
    loadSettings() {
        this.focusDuration = parseInt(localStorage.getItem('focusDuration') || '25') * 60;
        this.shortBreak = parseInt(localStorage.getItem('shortBreak') || '5') * 60;
        this.longBreak = parseInt(localStorage.getItem('longBreak') || '15') * 60;
    }
    ...
    complete() {
        ...
        if (this.sessionType === 'focus') {
            this.saveSession();
            const isLongBreak = this.sessions % 4 === 0;
            this.sessionType = isLongBreak ? 'longBreak' : 'shortBreak';
            this.duration = isLongBreak ? this.longBreak : this.shortBreak;
        } else {
            this.sessionType = 'focus';
    ...
    drawProgress() {
        ...
        ctx.strokeStyle = this.isRunning ? '#10b981' : '#6366f1';
        ctx.lineWidth = 15;
        ctx.lineCap = 'round';
        ctx.beginPath();
    ...
    showNotification() {
        if (Notification.permission === 'granted') {
            new Notification('Pomodoro Complete!', {
                body: this.sessionType === 'focus' ? 'Time for a break!' : 'Time to focus!',
    ...
    async saveSession() {
        if (window.storage) {
            await window.storage.add('pomodoro', {
                timestamp: Date.now(),
