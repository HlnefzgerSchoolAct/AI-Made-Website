...
class SoundGenerator {
    ...
    createForestSound() {
        this.sounds.forest = () => {
            ...
            return { source: osc, lfo };
        };
    }

    createBinauralBeats() {
        this.sounds.binaural = (freq1 = 200, freq2 = 210) => {
            const osc1 = this.audioContext.createOscillator();
            ...
            return { sources: [osc1, osc2], merger };
        };
    }

    play(soundName, volume = 0.5) {
        if (this.activeOscillators[soundName]) {
            this.stop(soundName);
        }
        ...
        if (sound.source) {
            ...
            sound.source.start();
            this.activeOscillators[soundName] = sound.source;
        } else if (sound.sources) {
            sound.sources.forEach(src => {
                src.connect(gainNode);
                src.start();
            });
            this.activeOscillators[soundName] = sound.sources;
        }
    ...
    stop(soundName) {
        const osc = this.activeOscillators[soundName];
        if (osc) {
            ...
            delete this.activeOscillators[soundName];
        }
    }
